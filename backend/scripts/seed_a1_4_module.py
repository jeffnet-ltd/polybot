#!/usr/bin/env python3
"""
MongoDB Seeding Script for Module A1.4: Ordering Food & Drinks
Populates the lessons collection with the structured A1.4 module.
"""
...
# Note: MODULE_A1_4_LESSONS is now imported from a1_4_module_data.py
# This ensures the seed script stays in sync with the main module data

async def seed_module_a1_4():
    """Seed Module A1.4 into MongoDB."""
    try:
        ...
        if existing:
            ...
            print(f"✅ Updated Module A1.4 (matched: {result.matched_count}, modified: {result.modified_count})")
        else:
            # Insert new module
        ...
        print(f"\n🎉 Module A1.4 seeding complete!")
        print(f"   - Module document in 'modules' collection")
        print(f"   - {len(MODULE_A1_4_LESSONS['lessons'])} lessons in 'lessons' collection")
        
        client.close()
        
    except Exception as e:
        print(f"❌ Error seeding Module A1.4: {e}")
        import traceback
        traceback.print_exc()
        sys.exit(1)
...
if __name__ == "__main__":
    print("=" * 60)
    print("PolyBot MongoDB Seeding Script - Module A1.4")
    print("=" * 60)
    asyncio.run(seed_module_a1_4())
