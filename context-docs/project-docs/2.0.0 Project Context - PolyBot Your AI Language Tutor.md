...
|**Text-to-Speech (TTS)**|**⚠️ Deprecated**|**Edge-TTS (Microsoft)**|**Unreliable due to DNS issues in containerized environments.** Was the primary TTS, now replaced by Azure.|
|**Text-to-Speech (TTS) Fallback**|**✅ Implemented**|**Azure Speech Services**|**Now the primary TTS solution.** Provides high availability and consistent voice quality. `TTSManager` implemented to handle this.|
...
- **TTS-Based Listening:** Listening comprehension exercises use TTS to read hidden sentences (not displayed to user) with replay functionality. **Now powered by Azure Speech due to `edge-tts` instability.**
...
- **Process:** Whisper STT → Llama 3 (with Game State) → Edge-TTS
- **Output:** Audio file
- **GPU Operations:** STT + LLM (expensive, isolated)
...
4. Audio Playback → Edge-TTS (Voice Mode) or Text Display (Text Mode)
...
1. **TTS High Availability (✅ Implemented):**
    
    **1.1 Implement `TTSManager` with Azure Fallback:**
    
    - **✅ Done:** Integrated Azure Speech Services as the primary TTS provider due to `edge-tts` unreliability.
    - **✅ Done:** Created a `TTSManager` class to handle the logic of trying `edge-tts` first and then falling back to Azure if `edge-tts` fails.
    - Securely manage Azure Speech API keys.
    
    **1.2 Update Endpoints:**
    
    - **✅ Done:** Refactored all TTS-related endpoints to use the new `TTSManager`.
    - **✅ Done:** Ensured seamless fallback with no disruption to the user experience.
...
- **TTSManager Class:** A new class to manage TTS services.
- **Primary TTS:** `edge-tts` was the primary service but is now deprecated due to DNS instability in containerized environments.
- **Fallback / Primary TTS:** Azure Speech Services is now the primary, reliable TTS provider.
- **Logic:** The manager will try to generate audio with `edge-tts` and, upon failure, will automatically switch to Azure Speech for that request.
