...
|**Text-to-Speech (TTS)**|**✅ Implemented**|**Edge-TTS (Microsoft)**|Primary TTS for audio playback of vocabulary and phrases. Provides high-quality, natural-sounding synthesis.|
|**Text-to-Speech (TTS) Fallback**|**🔄 Planned**|**Azure Speech Services**|Fallback TTS service to ensure high availability and consistent voice quality when Edge-TTS is unavailable.|
...
1. **TTS High Availability (Priority):**
    
    **1.1 Implement Azure Speech Fallback:**
    
    - Integrate Azure Speech Services as a fallback TTS provider.
    - Create a `TTSManager` class to handle the logic of trying `edge-tts` first and then falling back to Azure if `edge-tts` fails.
    - Securely manage Azure Speech API keys.
    
    **1.2 Update Endpoints:**
    
    - Refactor all TTS-related endpoints to use the new `TTSManager`.
    - Ensure seamless fallback with no disruption to the user experience.
...
- **Primary TTS:** `edge-tts` will be the primary, default TTS service.
- **Fallback TTS:** Azure Speech Services will be used as a fallback to ensure high availability.
